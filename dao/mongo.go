...
func (m *MongoService) GetPaymentsWithRefundStatus() ([]models.PaymentResourceDB, error) {
	...
	log.Info("Before reading data from mongoDB") // FIXME remove this temporary logging
	paymentDBResources, err := collection.Find(context.Background(), statusFilter)
	if err != nil {
		return nil, err
	}

	log.Info("After reading data from mongoDB") // FIXME remove this temporary logging
	err = paymentDBResources.All(context.Background(), &payments)
	log.Info("After converting mongo data to payments resource") // FIXME remove this temporary logging
	if err != nil {
		return nil, err
	}
